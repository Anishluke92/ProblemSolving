=begin
2. A positive number's population is the number of 1s in its binary representation. 
An evil number has an even numbered population, whereas an odious number has an odd numbered population.
...
Create a function that takes a number as an argument and
 returns a sorted array of all its descriptors ("Evil", "Odious", or "Pernicious").
Examples
...
def how_bad(n)
  ...
  if binary % 2 == 0 
    if !Prime.prime?(binary)
      result.push(numbered_population[0])
    else 
      result.push(numbered_population[0],numbered_population[2])
    end
  else 
    if !Prime.prime?(binary)
      result.push(numbered_population[1])
    else 
